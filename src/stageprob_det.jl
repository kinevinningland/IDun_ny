...
module StageProbDet
   ...
   function Build(t,iWeek,USMod,AHData,AMData,HSys,MCon,EV,CCR,CCH,CNS,NCut,NHSys,NArea,NLine,LineCap,LineLoss,CTI,LEndVal,LDemandResponse,DR,optimizer)
      ...
      for iArea in 1:NHSys, iMod in 1:AHData[iArea].NMod
         res0[iArea,iMod] = max(0.0, -DT*M3S2MM3*(dis[iArea,iMod,1]+byp[iArea,iMod,1]+spi[iArea,iMod,1]) 
      ...
      @variable(M, -NULL_BAL_TOL <= eps_resbal0[iArea=1:NHSys, iMod=1:AHData[iArea].NMod] <= NULL_BAL_TOL,
               base_name="eps_resbal0")

      # Slå av slack for vanlige magasin
      for iArea in 1:NHSys, iMod in 1:AHData[iArea].NMod
         if AHData[iArea].MData[iMod].MaxRes > NULL_RES_TOL
            fix(eps_resbal0[iArea,iMod], 0.0; force=true)
         end
      end
      ...
      @constraint(M, resbalReg0[iArea=1:NHSys, iMod=1:AHData[iArea].NMod],
         ...
         - DT*M3S2MM3*(sum(dis[iArea,USMod[iArea].USModA[iMod].Dis[iDisUS],1]
                           for iDisUS=1:USMod[iArea].USModA[iMod].NDis))
         - DT*M3S2MM3*(sum(byp[iArea,USMod[iArea].USModA[iMod].Byp[iBypUS],1]
                           for iBypUS=1:USMod[iArea].USModA[iMod].NByp))
         - DT*M3S2MM3*(sum(spi[iArea,USMod[iArea].USModA[iMod].Spi[iSpiUS],1]
